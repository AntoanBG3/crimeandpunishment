...
from character_module import Character, CHARACTERS_DATA, get_relationship_description
...
class Game:
            ...
            if rumor_text and not rumor_text.startswith("(OOC:"):
                self._print_color(f"\n{Colors.DIM}(You overhear some chatter nearby: \"{rumor_text}\"){Colors.RESET}", Colors.DIM)
                    if self.player_character: 
                        # --- Parse and Journal Actionable Clue from Rumor ---
                        clue_match = re.search(r"<CLUE>(.*?)</CLUE>", rumor_text)
                        if clue_match:
                            clue_id = clue_match.group(1)
                            # Remove the clue tag for cleaner journal entry, but keep context
                            context_for_journal = rumor_text.replace(clue_match.group(0), '').strip()
                            self.player_character.add_journal_entry("Actionable Clue", f"{context_for_journal} (Clue ID: {clue_id})", self._get_current_game_time_period_str())
                            self._print_color("(This seems like something you could follow up on. It's been noted in your journal.)", Colors.MAGENTA)
                        else:
            ...
            if not found_target:
                for npc in self.npcs_in_current_location:
                    if npc.name.lower().startswith(target_to_look_at):
                        ...
                        if self.gemini_api.model:
                            # --- Call new method for skill-influenced NPC observation ---
                            observation = self.gemini_api.get_character_observation_detail(
                                self.player_character, 
                                npc, 
                                self.current_location_name, 
                                self.get_current_time_period(), 
                            ...
                            # The old placeholder skill check is removed as get_character_observation_detail handles it.
                        else:
    ...
    def _handle_talk_to_command(self, argument):
        ...
        if target_npc:
            # --- NPC-NPC Relationship Behavior Check ---
            for other_npc in self.npcs_in_current_location:
                if other_npc.name != target_npc.name: # Don't check relationship with self
                    # Check target_npc's relationship towards other_npc
                    relationship_score = target_npc.npc_relationships.get(other_npc.name, 0)
                    if relationship_score < -3: # Strong negative relationship
                        self._print_color(f"{target_npc.name} glances warily at {other_npc.name} and seems unwilling to speak freely right now.", Colors.YELLOW + Colors.DIM)
                        return False, False # action_taken = False, show_atmospherics = False
            # --- End NPC-NPC Relationship Behavior Check ---
            ...
            while conversation_active:
                ...
                if self.gemini_api.model:
                    ...
                    ai_response = self.gemini_api.get_npc_dialogue(
                        ...
                        self._get_objectives_summary(target_npc),
                        self._get_objectives_summary(self.player_character),
                        all_npcs_in_location=self.npcs_in_current_location # Pass list of NPCs
                    )
    ...
    def _handle_read_item(self, item_to_use_name, item_props, item_obj_in_inventory):
        ...
        if item_to_use_name == "old newspaper" or item_to_use_name == "Fresh Newspaper":
            ...
            if article_snippet and not article_snippet.startswith("(OOC:"):
                ...
                # --- Parse and Journal Actionable Clue from Newspaper ---
                clue_match = re.search(r"<CLUE>(.*?)</CLUE>", article_snippet)
                if clue_match:
                    clue_id = clue_match.group(1)
                    context_for_journal = article_snippet.replace(clue_match.group(0), '').strip()
                    self.player_character.add_journal_entry("Actionable Clue", f"{context_for_journal} (Clue ID: {clue_id})", self._get_current_game_time_period_str())
                    self._print_color("(This seems like something you could follow up on. It's been noted in your journal.)", Colors.MAGENTA)
                # --- End Clue Parsing ---
