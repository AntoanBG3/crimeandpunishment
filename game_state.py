...
class Game:
    ...
    def run(self):
        ...
        while True:
            ...
            if self.gemini_api.model and self.current_location_name in ["Haymarket Square", "Tavern", "Squalid St. Petersburg Street"] and random.random() < AMBIENT_RUMOR_CHANCE_PUBLIC_PLACE:
                source_npc = random.choice(self.npcs_in_current_location) if self.npcs_in_current_location else Character("A Passerby", "A typical St. Petersburg citizen.", "", self.current_location_name, [])
            ...
            if command == "quit": self._print_color("Exiting game. Goodbye.", Colors.MAGENTA); break
                ...
                action_taken_this_turn = False; continue 
                ...
                is_general_look = (argument is None or argument.lower() in ["around", ""]) # "look around" is now a general look
                    ...
                    if not found_target:
                        ...
                        if is_scenery: 
                            ...
                            if self.gemini_api.model:
                                ...
                                if observation and not observation.startswith("(OOC:"): self._print_color(f"\"{observation}\"", Colors.CYAN)
                ...
                if not has_accessible_exits: print(f"{Colors.BLUE}There are no obvious exits from here.{Colors.RESET}")
                action_taken_this_turn = True 
                show_atmospherics_this_turn = True 
                ...
                if not argument: self._print_color("What do you want to take?", Colors.RED); action_taken_this_turn = False; show_atmospherics_this_turn = False
                elif not self.player_character: self._print_color("Cannot take items: Player character not available.", Colors.RED); action_taken_this_turn = False; show_atmospherics_this_turn = False
                else:
                ...
                if not argument: self._print_color("What do you want to drop?", Colors.RED); action_taken_this_turn = False; show_atmospherics_this_turn = False
                elif not self.player_character: self._print_color("Cannot drop items: Player character not available.", Colors.RED); action_taken_this_turn = False; show_atmospherics_this_turn = False
                else:
                ...
                if not self.player_character: self._print_color("Cannot use items: Player character not available.", Colors.RED); action_taken_this_turn = False; show_atmospherics_this_turn = False
                ...
                time_to_advance = TIME_UNITS_PER_PLAYER_ACTION * random.randint(3, 6) 
                ...
                if not argument: self._print_color("Who do you want to talk to?", Colors.RED); action_taken_this_turn = False
                    ...
                    if target_npc:
                        ...
                        action_taken_this_turn = True
                    else: self._print_color(f"You don't see anyone named '{target_name_input}' here.", Colors.RED); action_taken_this_turn = False
                show_atmospherics_this_turn = True 
                ...
                if not argument: self._print_color("Where do you want to move to?", Colors.RED); action_taken_this_turn = False
                    ...
                    if potential_target_loc_name:
                        ...
                        self.update_current_location_details(from_explicit_look_cmd=False) 
                        moved = True
                    else: self._print_color(f"You can't find a way to '{target_exit_input}' from here.", Colors.RED); action_taken_this_turn = False
                show_atmospherics_this_turn = moved 
            ...
            if action_taken_this_turn:
                ...
                if event_triggered and self.last_significant_event_summary and self.last_significant_event_summary not in self.key_events_occurred[-3:]:
                ...
                if self.gemini_api.model and command != "talk to" and self.time_since_last_npc_interaction >= TIME_UNITS_FOR_NPC_INTERACTION_CHANCE:
